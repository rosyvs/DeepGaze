...
def main(args):
    ...
    for l in label_list:
        for i in args.folds:
            ...
            name=f"{l}_informer"
            version = f"fold{i}"
            cmd = f'python eyemind/experiments/new_multitask_informer_comp.py -c {config} --fold_number {i} --model.encoder_ckpt {ckpt_path} --data.label_col {l} --trainer.logger.init_args.name {name} --trainer.logger.init_args.version {version}'
            print(cmd)
            cmd_list = cmd.split(" ")
